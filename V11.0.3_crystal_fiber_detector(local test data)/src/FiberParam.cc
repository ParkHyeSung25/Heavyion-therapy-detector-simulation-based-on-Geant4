...
FiberParam::FiberParam(const G4int numx_, const G4int numy_, const fiberSpec* spec)
: G4VPVParameterisation() {
  ...
  for (G4int copyNo=0; copyNo<numx*numy; copyNo++) {
    ...
    X.push_back( (column-numx/2)*20.052*mm + ( numx%2==0 ? 10.026*mm : 0 ) );
    Y.push_back( (row-numy/2)*20.052*mm + ( numy%2==0 ? 10.026*mm : 0 ) );
  }

  fFiberSpec = spec;
}

FiberParam::~FiberParam() {}

void FiberParam::ComputeTransformation(const G4int copyNo, G4VPhysicalVolume* physVol) const {
  physVol->SetTranslation(G4ThreeVector(X[copyNo],Y[copyNo],0.));
}

void FiberParam::ComputeDimensions(G4Tubs& fiber, const G4int copyNo, const G4VPhysicalVolume* physVol) const {
  G4int column = copyNo / numy;
  G4int row = copyNo % numy;
...
G4Material* FiberParam::ComputeMaterial(const G4int copyNo, G4VPhysicalVolume* physVol) const {
  G4int column = copyNo / numy;
  G4int row = copyNo % numy;
...
bool FiberParam::IsCerenkov(G4int col, G4int row) const {
  bool isCeren = false;
...
CellParam::CellParam(const FiberParam* fiberParam) {
  fFiberParam = fiberParam;
}

CellParam::~CellParam() {}

void CellParam::ComputeTransformation(const G4int copyNo, G4VPhysicalVolume* physVol) const {
