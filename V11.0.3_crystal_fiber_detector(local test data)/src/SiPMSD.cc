...
SiPMSD::SiPMSD(const G4String& name, const G4String& hitsCollectionName, std::pair<int,int> xy)
...
void SiPMSD::Initialize(G4HCofThisEvent* hce) {
  fHitCollection = new SiPMHitsCollection(SensitiveDetectorName,collectionName[0]);
...
G4bool SiPMSD::ProcessHits(G4Step* step, G4TouchableHistory*) {
  ...
  if (hit==NULL) {
    ...
    hit = new SiPMHit(fWavBin,fTimeBin);
  ...
  Interface::hitRange wavRange = findWavRange(energy);
  hit->CountWavlenSpectrum(wavRange);


  Interface::hitRange timeRange = findTimeRange(hitTime);
...
Interface::hitRange SiPMSD::findWavRange(G4double en) {
  int i = 0;
  for ( ; i < fWavBin+1; i++) {
...
Interface::hitRange SiPMSD::findTimeRange(G4double stepTime) {
  int i = 0;
  for ( ; i < fTimeBin+1; i++) {
